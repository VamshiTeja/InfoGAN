	...
	def build_graph(self,args):
		...
		with self.graph.as_default():
			...
			if(self.args.mode=="dc-gan"):
				...
				#define optimizers
				if(args.grad_clip==-1):
					self.D_solver = tf.train.RMSPropOptimizer(args.learning_rate).minimize(self.D_loss+self.D_reg, var_list=theta_D, global_step=self.global_step)
					self.G_solver = tf.train.RMSPropOptimizer(args.learning_rate).minimize(self.G_loss+self.G_reg, var_list=theta_G)
					self.Q_solver = tf.train.AdamOptimizer(args.learning_rate).minimize(self.Q_loss+self.Q_reg, var_list=theta_Q+theta_G)
				else:
					...
					grads, _ = tf.clip_by_global_norm(tf.gradients(self.Q_loss+self.Q_reg, theta_Q+theta_G),5)
